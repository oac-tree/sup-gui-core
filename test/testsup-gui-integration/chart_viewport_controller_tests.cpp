...
TEST_F(ChartViewportControllerTest, SetEmptyViewport)
{
  ...
  auto viewport_item = model.InsertItem<sup::gui::ChartViewportItem>();
...
TEST_F(ChartViewportControllerTest, SetNonEmptyViewport)
{
  ...
  mvvm::ApplicationModel model;
  auto viewport_item = model.InsertItem<sup::gui::ChartViewportItem>();
  auto line_series_item = model.InsertItem<sup::gui::LineSeriesItem>(viewport_item);
...
TEST_F(ChartViewportControllerTest, SetViewportWithLineSeries)
{
  ...
  // setting data to it
  auto array_item = sup::gui::CreatePlotData({{1.0, 10.0}, {2.0, 20.0}});
...
TEST_F(ChartViewportControllerTest, AddLineSeries)
{
  ...
  // validating axes
  auto attached_axes = line_series->attachedAxes();
...
TEST_F(ChartViewportControllerTest, AxisControllers)
{
  ...
  // setting data to it
  auto array_item = sup::gui::CreatePlotData({{1.0, 10.0}, {20.0, 200.0}});
  auto array_item_ptr = array_item.get();
  model.InsertItem(std::move(array_item), model.GetRootItem(), mvvm::TagIndex::Append());
  line_series_item->SetDataItem(array_item_ptr);
