...
void LineSeriesDataController::SetItem(const AnyValueItem *item)
{
  if (m_data_item == item)
  {
    return;
...
const AnyValueItem *LineSeriesDataController::GetDataItem() const
{
  return m_data_item;
}
...
void LineSeriesDataController::SetXOffset(double value)
{
  ...
  if (m_data_item)
  {
    ...
    for (auto point : m_data_item->GetChildren())
    {
      auto [x, y] = GetXY(*point);
...
{
  if (!m_data_item->IsArray())
  {
    throw std::runtime_error("Error in LineSeriesController: item can't represent time series");
  }
