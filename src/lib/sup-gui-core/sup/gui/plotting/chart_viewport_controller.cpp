...
void ChartViewportController::Subscribe()
{
  ...
  Connect<mvvm::ItemInsertedEvent>(this, &ChartViewportController::OnItemInsertedEvent);
  Connect<mvvm::AboutToRemoveItemEvent>(this, &ChartViewportController::OnAboutToRemoveItemEvent);
}
...
QtCharts::QLineSeries *ChartViewportController::SetupChartForLineSeries(LineSeriesItem *item)
{
  ...
  auto controller = std::make_unique<sup::gui::LineSeriesController>(qt_line_series);
...
void ChartViewportController::OnItemInsertedEvent(const mvvm::ItemInsertedEvent &event)
{
  const auto [parent, tagindex] = event;
  auto added_child = dynamic_cast<sup::gui::LineSeriesItem *>(parent->GetItem(tagindex));
...
void ChartViewportController::OnAboutToRemoveItemEvent(const mvvm::AboutToRemoveItemEvent &event)
{
  const auto [parent, tagindex] = event;
  auto child_about_to_be_removed =
      dynamic_cast<sup::gui::LineSeriesItem *>(parent->GetItem(tagindex));

  // looking for controller serving item
  auto on_element = [&](const std::unique_ptr<sup::gui::LineSeriesController> &cntrl) -> bool
